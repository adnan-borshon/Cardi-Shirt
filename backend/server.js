...
app.post("/api/analyze-ecg", async (req, res) => {
  try {
    ...
    const db = await getDb();
    const rows = db.exec(
      "SELECT waveform_data FROM ecg_sessions WHERE id=" + parseInt(id),
    );
    if (!rows.length || !rows[0].values.length)
      return res.status(404).json({ error: "Session not found" });
    const waveform = JSON.parse(rows[0].values[0][0] || "[]");
    let summary = "AI summary unavailable.";
    if (geminiModel) {
      const prompt = `You are CardiShirt AI. Analyze this ECG waveform data (first 1000 voltage points): ${JSON.stringify(waveform.slice(0, 1000))}. Write a concise clinical summary in 2 sentences.`;
      const aiRes = await geminiModel.generateContent(prompt);
...
app.get("/api/trends", async (req, res) => {
  try {
    ...
    const db = await getDb();
    const rows = db.exec(
      `SELECT DATE(timestamp) as day, AVG(bpm) as avgBpm, AVG(spo2) as avgSpo2, AVG(temp) as avgTemp FROM realtime_vitals WHERE timestamp >= datetime('now','-${days} days') GROUP BY DATE(timestamp) ORDER BY day ASC`,
    );
    if (!rows.length || !rows[0].values.length) return res.json([]);
    const data = rows[0].values.map((r) => ({
      day: r[0],
